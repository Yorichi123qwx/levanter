...
if (!args[0]) return conn.reply(m.chat, '*ابعت اللينك عشان اوريك بيتكلم عن اي🤔*', m)  
let user = global.db.data.users[m.sender]
try{
     await conn.sendMessage(m.chat, { image: { url: `https://image.thum.io/get/fullpage/${args[0]}` }, caption: `*طلبك يا حب🥰*` }, { quoted: m })
}
catch{
try {
...
webIMG = `https://api.screenshotmachine.com/?key=c04d3a&url=${args[0]}&screenshotmachine.com&dimension=${calidad}`*/
await conn.sendMessage(m.chat, { image: { url: krt.result }, caption: `*تفضل طلبك يا حب🥰*` }, { quoted: m }) 
//}
} catch { 
m.reply("Error.")
}}}
